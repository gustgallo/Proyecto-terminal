...
/**
 * Genera una contraseña temporal alfanumérica segura con caracteres especiales.
 * Creada para el sistema de recuperación de contraseña de la intranet.
 * 
 * @param int $length Longitud deseada de la contraseña (por defecto 10).
...
function generateTemporaryPassword(int $length = 10): string
{
    ...
    // Aseguramos al menos un carácter de cada tipo para máxima seguridad
    ...
    // Mezclamos los caracteres aleatoriamente para evitar patrones predecibles
...
try {
     ...
     * Descripción: Genera una contraseña temporal para el usuario y la envía por correo electrónico.
     * Acceso: Público (Protegido por reCAPTCHA v3).
     */
    if ($method === 'POST' && $path === '/auth/forgot-password') {
        $body = requestJson();
        $email = validateEmailValue($body['email'] ?? null);
        if ($email === null) {
            jsonResponse(['error' => 'Por favor ingresa un correo electrónico válido.'], 400);
        }
        ...
        // Buscamos al usuario activo por correo en MariaDB
        $user = dbOne($pdo, 'SELECT id, email, first_name, last_name, is_active FROM users WHERE email = ? LIMIT 1', [$email]);

        if ($user && boolValue($user['is_active'] ?? false)) {
            // Generamos la contraseña temporal de 10 caracteres
            $tempPassword = generateTemporaryPassword(10);
            $newHash = password_hash($tempPassword, PASSWORD_BCRYPT, ['cost' => 10]);

            // Actualizamos la contraseña en MariaDB
            dbExecute($pdo, 'UPDATE users SET password_hash = ? WHERE id = ?', [$newHash, (int) $user['id']]);

            // Enviamos el correo electrónico con los datos de acceso
            $to = $user['email'];
            $subject = "PRODISA SWGRHP-IG - Recuperacion de Contrasena Temporal";
            $message = "Hola " . $user['first_name'] . " " . $user['last_name'] . ",\n\n"
                     . "Has solicitado recuperar tu contraseña de acceso a la Intranet PRODISA.\n\n"
                     . "Tu nueva contraseña temporal es: " . $tempPassword . "\n\n"
                     . "Por favor inicia sesión con esta contraseña temporal y cámbiala desde la Intranet lo antes posible.\n\n"
                     . "Atentamente,\n"
                     . "Equipo de Sistemas PRODISA\n"
                     . "Comercializadora e Instalaciones Oriente Prodisa";

            $headers = "From: no-reply@prodisa.com.mx\r\n" .
                       "Reply-To: soporte@prodisa.com.mx\r\n" .
                       "X-Mailer: PHP/" . phpversion();

            @mail($to, $subject, $message, $headers);
        }

        // Respuesta estándar de seguridad
        jsonResponse([
            'message' => 'Si el correo ingresado se encuentra registrado en el sistema, se ha enviado una contraseña temporal a tu bandeja de entrada.'
        ]);
    }
    ...
    /**
     * Endpoint: PATCH /overtime/{id}/reopen
     * Descripción: Reabre una solicitud de horas extras autorizada o rechazada para corregir datos o incluir nueva evidencia.
     * Acceso: Exclusivo para Jefes de Logística/Producción, RRHH y Administrador Global.
     */
    if ($method === 'PATCH' && preg_match('#^/overtime/(\d+)/reopen$#', $path, $m)) {
        $user = requireAuth($config);
        requireRole($user, ['ADMIN_GLOBAL', 'ADMIN_RRHH', 'PRODUCCION']);
        $body = requestJson();
        $id = (int) $m[1];
        $record = dbOne($pdo, 'SELECT * FROM overtime_records WHERE id = ?', [$id]);
        if (!$record) {
            jsonResponse(['error' => 'Registro de horas extras no encontrado'], 404);
        }
        $reason = trim((string) ($body['reopenNotes'] ?? '')) ?: 'Solicitud reabierta para revisión por corrección o nueva evidencia.';
        
        // Reabrimos el estado a PENDIENTE_JEFE
        dbExecute($pdo, "UPDATE overtime_records SET status = 'PENDIENTE_JEFE', area_boss_notes = CONCAT('REABIERTO: ', ?), rrhh_notes = NULL WHERE id = ?", [$reason, $id]);
        
        $dateLabel = date('d/m/Y', strtotime((string) $record['start_date_time']));
        dbExecute($pdo, "INSERT INTO notifications (user_id, title, message, type) VALUES (?, ?, ?, 'OVERTIME_MODIFIED')", [
            (int) $record['user_id'],
            'Horas Extras Reabiertas para Revisión',
            "Tus horas extras del {$dateLabel} han sido reabiertas por supervisión. Motivo: {$reason}."
        ]);

        jsonResponse(['message' => 'Solicitud de horas extras reabierta exitosamente para nueva revisión.', 'record' => dbOne($pdo, 'SELECT * FROM overtime_records WHERE id = ?', [$id])]);
    }

    /**
     * Endpoint: PUT /overtime/{id}
     * Descripción: Permite al empleado editar su solicitud mientras permanezca en estatus PENDIENTE_JEFE.
     */
    if ($method === 'PUT' && preg_match('#^/overtime/(\d+)$#', $path, $m)) {
        $user = requireAuth($config);
        $id = (int) $m[1];
        $record = dbOne($pdo, 'SELECT * FROM overtime_records WHERE id = ?', [$id]);
        if (!$record) {
        ...
        // El empleado solo puede editar sus propias solicitudes en estatus PENDIENTE_JEFE
        if ((int) $record['user_id'] !== (int) $user['id'] && ($user['roleCode'] ?? '') !== 'ADMIN_GLOBAL') {
            jsonResponse(['error' => 'No tienes permiso para editar este registro.'], 403);
        }

        if ((string) $record['status'] !== 'PENDIENTE_JEFE' && ($user['roleCode'] ?? '') !== 'ADMIN_GLOBAL') {
            jsonResponse(['error' => 'Solo se pueden editar solicitudes que estén en estatus PENDIENTE_JEFE.'], 400);
        }
        ...
        $hours = round(($endTs - $startTs) / 3600, 2);
        dbExecute($pdo, 'UPDATE overtime_records SET start_date_time = ?, end_date_time = ?, hours_count = ?, approved_hours_count = ?, justification = ?, project_id = ? WHERE id = ?', [
            $start, $end, $hours, $hours, $justification, !empty($body['projectId']) ? (int) $body['projectId'] : null, $id
        ]);

        jsonResponse(['message' => 'Solicitud de horas extras actualizada exitosamente.', 'record' => dbOne($pdo, 'SELECT * FROM overtime_records WHERE id = ?', [$id])]);
    }

    /**
     * Endpoint: DELETE /overtime/{id}
     * Descripción: Permite al empleado eliminar su solicitud mientras permanezca en estatus PENDIENTE_JEFE.
     */
    if ($method === 'DELETE' && preg_match('#^/overtime/(\d+)$#', $path, $m)) {
        $user = requireAuth($config);
        $id = (int) $m[1];
        $record = dbOne($pdo, 'SELECT * FROM overtime_records WHERE id = ?', [$id]);
        if (!$record) {
        ...
        if ((int) $record['user_id'] !== (int) $user['id'] && ($user['roleCode'] ?? '') !== 'ADMIN_GLOBAL') {
            jsonResponse(['error' => 'No tienes permiso para eliminar este registro.'], 403);
        }

        if ((string) $record['status'] !== 'PENDIENTE_JEFE' && ($user['roleCode'] ?? '') !== 'ADMIN_GLOBAL') {
            jsonResponse(['error' => 'Solo se pueden eliminar solicitudes que estén en estatus PENDIENTE_JEFE.'], 400);
        }

        dbExecute($pdo, 'DELETE FROM overtime_records WHERE id = ?', [$id]);
        jsonResponse(['message' => 'Solicitud de horas extras eliminada exitosamente.']);
    }
    ...
    /**
     * Endpoint: GET /roles
     * Descripción: Obtiene el catálogo de roles para asignación de usuarios.
     * Acceso: Exclusivo para Administrador Global (ADMIN_GLOBAL).
     */
    if ($method === 'GET' && $path === '/roles') {
        $user = requireAuth($config);
        requireRole($user, ['ADMIN_GLOBAL']);
    ...
    // =========================================================================
    // CHATBOT Y GESTIÓN DE DOCUMENTOS RAG (COHERE AI)
    // =========================================================================

    /**
     * Endpoint: GET /chat/rag/documents
     * Descripción: Obtiene el listado de documentos RAG registrados en MariaDB.
     * Acceso: Exclusivo para ADMIN_GLOBAL y ADMIN_RRHH.
     */
    if ($method === 'GET' && $path === '/chat/rag/documents') {
        $user = requireAuth($config);
        requireRole($user, ['ADMIN_GLOBAL', 'ADMIN_RRHH']);
        $rows = dbAll($pdo, "SELECT d.*, CONCAT_WS(' ', u.first_name, u.last_name) AS uploader_name,
                                    (SELECT COUNT(*) FROM rag_document_chunks c WHERE c.document_id = d.id) AS chunks_count
    ...
    /**
     * Endpoint: POST /chat/rag/documents
     * Descripción: Registra un nuevo documento o reglamento para alimentar el conocimiento del Chatbot RAG.
     * Acceso: Exclusivo para ADMIN_GLOBAL y ADMIN_RRHH.
     */
    if ($method === 'POST' && $path === '/chat/rag/documents') {
        $user = requireAuth($config);
        requireRole($user, ['ADMIN_GLOBAL', 'ADMIN_RRHH']);
        $body = requestJson();
        $title = trim((string) ($body['title'] ?? ''));
        $category = trim((string) ($body['category'] ?? 'Políticas Institucionales'));
        $content = trim((string) ($body['content'] ?? ''));

        if (textLength($title) < 3 || textLength($content) < 10) {
            jsonResponse(['error' => 'Por favor ingresa un título descriptivo y el contenido del documento RAG.'], 400);
        }

        $pdo->beginTransaction();
        try {
            try {
                $stmt = $pdo->prepare("INSERT INTO rag_documents (title, category, content, file_path, file_type, is_active, uploaded_by) VALUES (?, ?, ?, 'TEXT_CONTENT', 'TXT', 1, ?)");
                $stmt->execute([$title, $category, $content, (int) $user['id']]);
            } catch (Throwable $t) {
                $stmt = $pdo->prepare("INSERT INTO rag_documents (title, category, file_path, file_type, is_active, uploaded_by) VALUES (?, ?, 'TEXT_CONTENT', 'TXT', 1, ?)");
                $stmt->execute([$title, $category, (int) $user['id']]);
            }
            $docId = (int) $pdo->lastInsertId();

            // Fragmentamos por párrafos/secciones con solapamiento. Esto conserva el
            // significado de cada bloque y evita cortes arbitrarios cada 800 caracteres.
            $chunks = ragChunkDocument($content);
            if (!$chunks) {
                throw new RuntimeException('No fue posible generar fragmentos útiles del documento.');
            }

            $chunkStmt = $pdo->prepare("INSERT INTO rag_document_chunks (document_id, chunk_index, content) VALUES (?, ?, ?)");
            foreach ($chunks as $index => $chunkText) {
                $chunkStmt->execute([$docId, $index, $chunkText]);
            }

            $pdo->commit();
            $doc = dbOne($pdo, 'SELECT * FROM rag_documents WHERE id = ?', [$docId]);
            jsonResponse(['message' => 'Documento RAG agregado exitosamente al conocimiento del Chatbot.', 'document' => $doc], 201);
        } catch (Throwable $e) {
    ...
    /**
     * Endpoint: DELETE /chat/rag/documents/{id}
     * Descripción: Elimina un documento RAG y sus fragmentos asociados del sistema.
     * Acceso: Exclusivo para ADMIN_GLOBAL y ADMIN_RRHH.
     */
    if ($method === 'DELETE' && preg_match('#^/chat/rag/documents/(\d+)$#', $path, $m)) {
        $user = requireAuth($config);
        requireRole($user, ['ADMIN_GLOBAL', 'ADMIN_RRHH']);
        $id = (int) $m[1];
        $doc = dbOne($pdo, 'SELECT * FROM rag_documents WHERE id = ?', [$id]);
        if (!$doc) {
            jsonResponse(['error' => 'Documento RAG no encontrado.'], 404);
        }
        dbExecute($pdo, 'DELETE FROM rag_documents WHERE id = ?', [$id]);
        jsonResponse(['message' => 'Documento RAG eliminado exitosamente.']);
    }

    /**
     * Endpoint: POST /chat/query
     * Descripción: Procesa la consulta del empleado buscando contexto RAG en MariaDB y llamando a Cohere AI API.
     */
    if ($method === 'POST' && $path === '/chat/query') {
        ...
        if ($message === '') {
            jsonResponse(['error' => 'La pregunta no puede estar vacía.'], 400);
        }
        ...
        // Cargamos el historial reciente ANTES de guardar la pregunta actual.
        // El historial se usa solo para continuidad conversacional, nunca como
        // evidencia de políticas o procedimientos.
        $history = dbAll(
            $pdo,
        ...
        $stmt = $pdo->prepare("INSERT INTO chat_messages (conversation_id, sender, content) VALUES (?, 'USER', ?)");
        $stmt->execute([$conversationId, $message]);

        // Pipeline único del chatbot:
        // 1) recuperación léxica con filtro temático,
        // 2) reranking semántico opcional con Cohere,
        // 3) verificación de suficiencia dentro del prompt,
        // 4) respuesta natural o abstención segura.
        $evidence = ragRetrieveEvidence($pdo, $message, $history, $config);
        $answer = callCohereAiService($config, $message, $evidence, $history);

        // Guardamos la respuesta generada por el Bot
        $stmt = $pdo->prepare("INSERT INTO chat_messages (conversation_id, sender, content) VALUES (?, 'BOT', ?)");
        $stmt->execute([$conversationId, $answer]);
